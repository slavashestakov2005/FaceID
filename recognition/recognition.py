...
def detect_faces_cv(d, image):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    return d.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=5, minSize=(30, 30), flags=cv2.CASCADE_SCALE_IMAGE)


def detect_faces_numpy_cv(detector, img_numpy):
    return detector.detectMultiScale(img_numpy)
